...
if (isset($_POST['addCustomer'])) {
  ...
  if (empty($_POST["cfirst"]) || empty($_POST["clast"]) || empty($_POST['cgender']) || empty($_POST['ccontact']) || empty($_POST['cage']) || empty($_POST['cstreet']) || empty($_POST['ccity']) || empty($_POST['cprovince'])) {
    ...
    $postQuery = "INSERT INTO buyer (address_id, contact, first_name, last_name, gender, age) VALUES(?,?,?,?,?,?)";
    $postStmt = $mysqli->prepare($postQuery);
    //bind paramaters
    $rc = $postStmt->bind_param('ssssss', $currentID, $contact, $first_name, $last_name, $gender, $age);
    $postStmt->execute();
